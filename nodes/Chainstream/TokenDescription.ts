...
export const tokenFields: INodeProperties[] = [
...

  {
    ...
    displayOptions: { show: { operation: ['get'], resource: ['token'] } },
    default: {},
    options: [
      {
        displayName: 'Fields',
        ...
        type: 'string',
        default: '',
        description: 'Fields the token will return, formatted as a string of comma-separated values. By default all the fields are returned.',
      },
    ],
  },
  ...
  /* token:getMany */
  {
    ...
    // eslint-disable-next-line n8n-nodes-base/node-param-type-options-password-missing
    typeOptions: { password: false },
    displayOptions: { show: { operation: ['getMany'], resource: ['token'] } },
  },
  ...
  /* token:holders pagination (kept commented for reference) */
  /*{
    displayName: 'Cursor',
    name: 'cursor',
    type: 'string',
  ...
  /* token:candles params (kept commented) */
  /*{
    ...
    displayOptions: { show: { resource: ['token'], operation: ['candles'] } },
  },
  {
  ...
  /* token:prices pagination (kept commented) */
  /*{
    displayName: 'Prices Limit',
    name: 'pricesLimit',
    type: 'number',
    default: 100,
    displayOptions: { show: { resource: ['token'], operation: ['prices'] } },
    typeOptions: { minValue: 1 },
    description: 'Limit for prices endpoint (separate from search limit)',
  },*/

  /* token:price (kept commented) */
  /*{
    ...
    displayOptions: { show: { resource: ['token'], operation: ['price'] } },
  },*/

  /* token:mintAndBurn filters and pagination (kept commented) */
  /*{
    displayName: 'Type',
    name: 'type',
    type: 'string',
    ...
    displayOptions: { show: { resource: ['token'], operation: ['mintAndBurn'] } },
  },*/
  ...
  /*{
    ...
    type: 'number',
    default: 10,
    displayOptions: { show: { resource: ['token'], operation: ['topHolders'] } },
